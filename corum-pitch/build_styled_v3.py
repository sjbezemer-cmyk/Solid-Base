...
def _dash(shape):
    return  # STYLED: geen wireframe-stippellijnen


def box(s, x, y, w, h, fill=LIGHT, line=None, dashed=False):
    sh = s.shapes.add_shape(MSO_SHAPE.RECTANGLE, x, y, w, h)
    sh.fill.solid(); sh.fill.fore_color.rgb = fill
    sh.line.color.rgb = line if line is not None else HAIR
    sh.line.width = Pt(0.75 if line is not None else 0.5)
...
def label(s, x, y, w, text):
    # STYLED: interne build-/design-notities eruit; alleen echte planning-caveats behouden
    low = text.strip().lower()
    if 'indicat' not in low:
        return
    t = text.strip()
    if low.startswith('wireframe:'):
        t = t.split(':', 1)[1].strip()
    txt(s, x, y, w, Inches(0.3), t, size=8, color=GREY, italic=True)
...
prs.save("corum-pitch/Corum-Westgate-I-Pitch-STYLED.pptx")
